...
# ---------- Paths ----------
MODEL_PATH = r"C:\Users\Mokesh\Desktop\hackathon\model\vosk-model-small-hi-0.22\vosk-model-small-hi-0.22"
QA_PATH = r"C:\Users\Mokesh\Desktop\hackathon\qa.json"
...
def humanize(text):
    """
    Adds slight pauses for smoother speech.
    """
    text = text.replace("।", ". ")
    text = text.replace(",", ", ")
    text = text.replace("?", "? ")
...
def speak(text):
    """
    Male Hindi voice, normal speed, clear and smooth.
    """
    global LISTENING
    LISTENING = False
    ...
    with open(tmp, "w", encoding="utf-8") as f:
        f.write(humanize(text))

    # --- Using your preferred settings ---
    subprocess.run(
        [
            ESPEAK,
            "-v", "hi",       # male Hindi voice
            "-s", "145",      # normal speed
            "-p", "40",       # pitch
            "-a", "170",      # volume
            "-g", "10",       # word gap
...
tk.Label(
    root,
    text="Offline Hindi Voice Assistant",
...
tk.Label(
    root,
    text="Assistant Reply (Live Output)",
    font=("Segoe UI", 12, "bold")
